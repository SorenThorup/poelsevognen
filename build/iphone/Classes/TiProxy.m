...
//Rare exceptions to indicate a bug in the _poelsevognen code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_poelsevognen = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_poelsevognen]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.poelsevognen.module"
